...
loop = True
####################################### 검색어 리스트 #################################
search_key_list = ['중구 중림동 카페','중구 소공동 카페','중구 회현동2가 카페','중구 명동 카페','중구 필동 카페','중구 을지로동 카페','중구 광희동 카페','중구 장충동 카페','중구 다산동 카페','중구 신당동 카페','중구 황학동 카페','중구 신당제5동 카페','중구 동화동 카페','중구 청구동 카페','중구 약수동 카페']
for search_key in search_key_list:
    ...
    while(loop):
        ...
        for index, e in enumerate(elemets, start=1):
            ...
            # 저장할 파일명
            output_file = f"{search_key}_log{page_no}.txt"
        ...
        Df.to_csv(f'{search_key}_{page_no}.csv', index=True)
